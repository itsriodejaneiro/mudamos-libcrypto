...
module.exports = (function(){
    function createWalletPrivate(seed) {
        if (!seed)
            throw new LibCriptoError(listErros['SeedEmptyError']);
    ...
    class Wallet {
        ...
        static signMessage(seed, message) {
            if (!seed)
                throw new LibCriptoError(listErros['SeedEmptyError']);
            if (!message)
                throw new LibCriptoError(listErros['MessageEmptyError']);
        ...
        static verifyMessage(publicKey, message, signature) {
            if (!publicKey)
                throw new LibCriptoError(listErros['PublicKeyEmptyError']);

            if (!message)
                throw new LibCriptoError(listErros['MessageEmptyError']);

            if (!signature)
                throw new LibCriptoError(listErros['SignatureEmptyError']);
